...
char *gi_show(FILE *fp)
{
  ...
  for (b = fgetc(fp); !feof(fp); b = fgetc(fp)) {
    ...
    case 0xce:/* * sign */ 
    case 0xcf:/* / sign */ 
    case 0xd4: { /* = sign; this is a numeric operation, try to read a number */
      int c;
      if ((c = fgetc(fp)) > 0x80) {
	ungetc(c, fp);
...
Program *gi_open(const char *fname)
{
  ...
  /* There is some junk at the beginning of ORIC tape images */ 
  for (i=1;i<=18;i++) fgetc(bprog); 
  if (fgetc(bprog) != 0x24) {
    printf("oricascii: failed to find Oric magic cookie\n");
    fclose(bprog);
    free(p);
    return (Program *)NULL;
  }
