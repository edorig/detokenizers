...
char *gi_show(FILE *fp)
{
  ...
  for (b = fgetc(fp); !feof(fp); b = fgetc(fp)) {
    ...
    case 0x0e: /* a number designator, used for GOTO and GOSUB */ 
      /* Big-Endian representation since the chip is a Motorola 6809 */ 
      x += sprintf(&buf[x], "%d", ((fgetc(fp) <<8) | fgetc(fp)));
      break;
    case 0xf:
    ...
    case 0xae:
    case 0xaf: 
    case 0xb3: { /* this is a numeric operation, try to read a number */
      int c;
      if ((c = fgetc(fp)) > 0x80) {
	ungetc(c, fp);
	...
	case '\"':
	  x += sprintf(&buf[x], "%s%c%c", gi_futz_byte(b, gwb_ops), c,
		       fgetc(fp));
	  break;
	  /* We assume that since the CoCo is using a Motorola 6809 chip, words are in big endian format with MSB first, LSB second */ 
	case 0xc: {
	  int hw1 = fgetc(fp),
	      hw2 = fgetc(fp);
	...
	case 0x1c:
	   x += sprintf(&buf[x], "%s%d", gi_futz_byte(b, gwb_ops),(fgetc(fp) <<8 | (fgetc(fp)))); 
	   break; 
	case 0x1d:  
	case 0x1f:
	  {
	    int hw1 = fgetc(fp),
	        ...
	        hw3 = fgetc(fp),
	        hw4 = fgetc(fp);
	    /* We also assume big-endian representation: exponent first, then MSB of Mantissa to LSB of Mantissa */ 
	    if (c != 0x1f) {
	      float f1 = 0.0, f2 = 0.0, f = 0.0;
	      
	      f1 = (float)(((float)hw3 * (float)gi_power(2.0, -15)) +
			   ((float)hw4 * (float)gi_power(2.0, -23)) +
			   ((float)hw2 * (float)gi_power(2.0, -7)) + 1.0);
	      f2 = (float)gi_power(2.0, (hw1 - 129));
...
Line *gi_parse(FILE *fp)
{
  ...
  /* the CoCo is based on a Motorola 6809 chip, so line number is in Big Endian format. The first byte is the most significant byte (MSB) the second byte is the least significant byte */ 
  lineno = ((cb[2]<<8) | cb[3]);
