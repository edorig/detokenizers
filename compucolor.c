...
char *gi_show(FILE *fp)
{
  ...
  for (b = fgetc(fp); !feof(fp); b = fgetc(fp)) {
	...
	int hw1 = fgetc(fp),
	  ...
	  hw3 = fgetc(fp),
	  hw4 = fgetc(fp);
	/* We are assuming that the Compucolor used single precision floating point since the BASIC manual indicates 6 digits with exponents ranging from E-38 to E+38 */ 	
	if (b != 0x1f) {
